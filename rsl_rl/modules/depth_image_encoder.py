...
class RecurrentDepthBackbone(nn.Module):
    def __init__(self, base_backbone, num_prop) -> None:
        ...
        self.combination_mlp = nn.Sequential(
                                nn.Linear(base_backbone.output_dim + num_prop, 128),
                                activation,
                                nn.Linear(128, 32)
                                )
        self.rnn = nn.GRU(input_size=32, hidden_size=512, batch_first=True)
        self.output_mlp = nn.Sequential(
                            nn.Linear(512, base_backbone.output_dim),
                            last_activation
                            )
    ...
    def reset_hidden_states(self, done_mask=None):
        if done_mask is not None:
            self.hidden_states[:, done_mask, :] = 0
...
class DepthOnlyFCBackbone58x87(nn.Module):
    def __init__(self, scandots_output_dim, hidden_state_dim, output_activation=None, num_frames=1):
        super().__init__()
        self.output_dim = scandots_output_dim
