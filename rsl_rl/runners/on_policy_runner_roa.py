...
import rsl_rl
from rsl_rl.algorithms import PPOROA
...
class OnPolicyRunnerROA:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if "private" in extras["observations"]:
            num_private_obs = extras["observations"]["private"].shape[1]
        else:
            num_private_obs = 0
        ...
        actor_critic: ActorCriticROA = actor_critic_class(
            num_obs, num_critic_obs, num_private_obs, num_history_obs, self.env.num_actions, **self.policy_cfg
        ).to(self.device)

        # init algorithm
        alg_class = eval(self.alg_cfg.pop("class_name"))  # PPOROA
        self.alg: PPOROA = alg_class(actor_critic, device=self.device, **self.alg_cfg)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):
        ...
        history_obs = self.history_obs_normalizer(extras["observations"]["history"].to(self.device))
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    actions = self.alg.act(obs, critic_obs, private_obs, history_obs, hist_encoding)
                    # Step environment
                    ...
                    #     critic_obs = obs
                    critic_obs = self.critic_obs_normalizer(infos["observations"]["critic"].to(self.device))
                    private_obs = self.private_obs_normalizer(infos["observations"]["private"].to(self.device))
                ...
                # Learning step
                start = stop
                self.alg.compute_returns(critic_obs, private_obs)
            ...
            if hist_encoding:
                ...
                mean_value_loss, mean_surrogate_loss, mean_entropy, mean_priv_reg_loss, priv_reg_coef = self.alg.update()
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            log_string = (
                ...
                f"""{'Private Regular loss:':>{pad}} {locs['mean_priv_reg_loss']:.4f}\n"""
                f"""{'Private Regular coefficience:':>{pad}} {locs['priv_reg_coef']:.4f}\n"""
            )
    ...
    def get_inference_policy(self, device=None):
        ...
        def policy(observations, private_observations=None, history_observations=None, hist_encoding=None):
            # 可选地做归一化
            if self.cfg["empirical_normalization"]:
                if device is not None:
                    self.obs_normalizer.to(device)
                observations = self.obs_normalizer(observations)
                private_observations = self.private_obs_normalizer(private_observations)
