...
class PPO:
    ...
    def __init__(
        ...
        # Estimator component
        if hasattr(actor_critic, "estimator") and actor_critic.estimator is not None:
    ...
    def update_dagger(self, teacher_buffer, student_buffer):
        mean_hist_latent_loss = 0
        student_buffer = self.actor_critic.infer_priv_latent(student_buffer)
        loss = (teacher_buffer.detach() - student_buffer).norm(p=2, dim=1).mean()
