...
parser.add_argument(
    "--if_depth", action="store_true", default=False, help="If use depth camera during training."
)
...
def main():
    ...
    ppo_runner = OnPolicyRunnerRMA(env, agent_cfg.to_dict(), log_dir=log_dir, device=agent_cfg.device)
    ppo_runner.load(resume_path)

    # obtain the trained policy for inference
    if args_cli.if_depth:
        policy = ppo_runner.get_inference_policy_vision(device=env.unwrapped.device)
    else:
        policy = ppo_runner.get_inference_policy_scan(device=env.unwrapped.device)
    ...
    if args_cli.if_depth:
        other_obs = extras["observations"]["vision"].to(agent_cfg.device)
    else:
        other_obs = extras["observations"]["scan"].to(agent_cfg.device)
