...
import omni
import rsl_rl_utils
from rsl_rl.runners import OnPolicyRunnerVAE
...
from vae_exporter import export_vae_policy_as_jit
# sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
...
def main():
    ...
    if args_cli.keyboard:
        ...
        system_input.subscribe_to_keyboard_events(
            ...
            lambda event: rsl_rl_utils.sub_keyboard_event(event, cmd_vel, lin_vel=0.1, ang_vel=0.1),
        )
    ...
    ppo_runner = OnPolicyRunnerVAE(env, agent_cfg.to_dict(), log_dir=log_dir, device=agent_cfg.device)
    ...
    # reset environment
    obs_dict, extras = env.get_observations()
    # history_obs_dict = extras["observations"]["history"]
    obs, history_obs = ppo_runner.process_history_term_first(obs_dict, agent_cfg.policy.hist_step, env.num_envs)
    obs, history_obs = obs.to(agent_cfg.device), history_obs.to(agent_cfg.device)
    timestep = 0
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            # agent stepping
            actions = policy(obs, history_obs)
            # env stepping
            obs_dict, _, _, extras = env.step(actions)
            # history_obs = extras["observations"]["history"].to(agent_cfg.device)
            # obs, history_obs = ppo_runner.process_history_term_first(history_obs, agent_cfg.policy.hist_step, env.num_envs).to(agent_cfg.device)
            # history_obs_dict = extras["observations"]["history"]
            obs, history_obs = ppo_runner.process_history_term_first(obs_dict, agent_cfg.policy.hist_step, env.num_envs)
            obs, history_obs = obs.to(agent_cfg.device), history_obs.to(agent_cfg.device)
