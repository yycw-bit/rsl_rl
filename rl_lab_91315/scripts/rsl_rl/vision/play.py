...
def main():
    ...
    depth_img = camera.data.output["depth"][5].squeeze(-1).cpu().numpy()  # [H, W]
    plt.ion()  # 开启交互模式
    fig, ax = plt.subplots()
    im = ax.imshow(depth_img, cmap="gray")
    ...
    while simulation_app.is_running():
        ...
        depth_img = camera.data.output["depth"][0].squeeze(-1).cpu().numpy()  # [H, W]
        print("depth_img.min()", depth_img.min())
        print("depth_img.max()", depth_img.max())
        # depth_vis = (depth_img - depth_img.min()) / (depth_img.max() - depth_img.min() + 1e-6)
        # # depth_vis = (depth_vis * 255).astype(np.uint8)
        # # cv2.imshow("Depth", depth_vis)
        # # cv2.waitKey(1)
        # depth_plot.set_data(depth_vis)
        # plt.pause(0.01)
        im.set_data(depth_img)
        ax.set_title(f"Step")
        plt.pause(0.001)  # 非阻塞，继续仿真
