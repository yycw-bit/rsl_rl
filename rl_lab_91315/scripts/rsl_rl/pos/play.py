...
def main():
    ...
    env_cfg.commands.base_position.debug_vis = True
    # env_cfg.commands.base_position.ranges.lin_vel_x = (-1.5, 1.5)
    # env_cfg.commands.base_position.ranges.lin_vel_y = (-0.5, 0.5)
    # env_cfg.commands.base_position.ranges.ang_vel_z = (-1.0, 1.0)
    # env_cfg.commands.base_position.ranges.heading = (-0.1, 0.1)
    ...
    if args_cli.keyboard:
        ...
        # env_cfg.commands.base_position.debug_vis = False
        cmd_pos = torch.zeros((env_cfg.scene.num_envs, 2), dtype=torch.float32)
        mode_boolean = torch.zeros((env_cfg.scene.num_envs, 1), dtype=torch.float32)
        system_input = carb.input.acquire_input_interface()
        system_input.subscribe_to_keyboard_events(
            omni.appwindow.get_default_app_window().get_keyboard(),
            lambda event: rsl_rl_utils.sub_keyboard_event(event, cmd_pos, mode_boolean, pos_x=0.5, pos_y=0.5),
        )
        env_cfg.observations.policy.position_commands = ObsTerm(
            func=lambda env: cmd_pos.clone().to(env.device),
        )
        env_cfg.observations.policy.mode_boolean = ObsTerm(
            func=lambda env: mode_boolean.clone().to(env.device),
        )
