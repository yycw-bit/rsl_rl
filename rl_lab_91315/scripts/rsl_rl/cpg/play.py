...
from rsl_rl.env import ManagerBasedRLCPGEnv, RslRlVecEnvWrapperCPG
...
def main():
    ...
    env_cfg.commands.base_velocity.ranges.lin_vel_y = (-0., 0.)
    ...
    joint_index = 11  # which joint is used for logging
    ...
    foot_xs = np.stack(env.unwrapped.foot_xs_list, axis=0)  # [T, num_envs, 4]
    foot_ys = np.stack(env.unwrapped.foot_ys_list, axis=0)
    foot_zs = np.stack(env.unwrapped.foot_zs_list, axis=0)

    fig = plt.figure(figsize=(12, 10))  # 设置图像大小

    num_legs = 4
    env_id = 0  # 指定要显示的环境编号

    for leg_id in range(num_legs):
        ax = fig.add_subplot(2, 2, leg_id + 1, projection='3d')  # 创建 2x2 的子图布局
        x_traj = foot_xs[:, env_id, leg_id]
        y_traj = foot_ys[:, env_id, leg_id]
        z_traj = foot_zs[:, env_id, leg_id]

        ax.plot(x_traj, y_traj, z_traj, label=f'Leg {leg_id}')
        ax.set_title(f'Foot Trajectory - Leg {leg_id}')
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.legend()
