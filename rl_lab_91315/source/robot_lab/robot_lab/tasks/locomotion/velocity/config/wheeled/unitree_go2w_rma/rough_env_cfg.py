...
@configclass
class UnitreeGo2WMySceneCfg(MySceneCfg):
    """Scene specifications for the MDP."""
    depth_camera = TiledCameraCfg(
        prim_path="{ENV_REGEX_NS}/Robot/base/front_camera",
        offset=TiledCameraCfg.OffsetCfg(pos=(0.27, 0, 0.03),  convention="world"),
        update_period = 2,
        height = 60,
        width = 106,
        data_types = ["depth"],
        # debug_vis = True,
        spawn = sim_utils.PinholeCameraCfg(focal_length=10.98, focus_distance=11.8, horizontal_aperture=20.955, clipping_range=(0.105, 2)),
        # spawn=None,
...
class UnitreeGo2WObservationCfg(ObservationsCfg):
    ...
    class VisionCfg(ObsGroup):
        depth_images = ObsTerm(func=mdp.camera_scan,
...
class UnitreeGo2WRoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    scene: UnitreeGo2WMySceneCfg = UnitreeGo2WMySceneCfg(num_envs=4096, env_spacing=3.0)  # 2.5
    ...
    def __post_init__(self):
        ...
        # self.scene.vision_camera.prim_path = "{ENV_REGEX_NS}/Robot/" + self.base_link_name
        # self.scene.vision_camera = None
        ...
        self.observations.scan.height_scan.scale = 5.0  # 0.5
        self.observations.vision.depth_images.scale = 1.0
        # ------------------------------Actions------------------------------
        # reduce action scale
