...
class UnitreeGo2WObservationsCfg:
    ...
    class CriticCfg(ObsGroup):
        ...
        contact_state = ObsTerm(func=mdp.true_contact, params={"sensor_cfg": SceneEntityCfg("contact_forces")},
    ...
    @configclass
    class OthersCfg(ObsGroup):
        desired_contact = ObsTerm(func=mdp.desired_contact,
                                  params={"cycle_time": 1., "offsets": 0., "bounds": 0., "durations": 0.5,
                                          "phases": 0.5},
                                  scale=1.0,
                                  clip=(-100.0, 100.0))
    ...
    others: OthersCfg = OthersCfg()
    privillege: Privillege = Privillege()
...
class UnitreeGo2WRewardsCfg(RewardsCfg):
    ...
    joint_track_error = RewTerm(
        func=mdp.joint_track_error, weight=0.0, params={"asset_cfg": SceneEntityCfg("robot", joint_names=".*")},
    )
    wheel_track_error = RewTerm(
        func=mdp.wheel_track_error, weight=0.0, params={"asset_cfg": SceneEntityCfg("robot", joint_names="")}
    )
    tracking_contacts_shaped = RewTerm(
        func=mdp.tracking_contacts_shaped, weight=0.0, params={ "asset_cfg": SceneEntityCfg("robot"), "sensor_cfg": SceneEntityCfg("contact_forces")}
    )


@configclass
class UnitreeGo2WRoughEnvMlpWithEstimatorCfg(LocomotionVelocityRoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.observations.policy.joint_pos.params["asset_cfg"] = SceneEntityCfg(
                                                                "robot", joint_names= [f"^(?!{self.wheel_joint_name}).*"])
        ...
        self.observations.policy.joint_pos_history.func = mdp.joint_pos_rel_without_wheel
        self.observations.policy.joint_pos_history.params["asset_cfg"] = SceneEntityCfg(
            "robot", joint_names=[f"^(?!{self.wheel_joint_name}).*"])

        self.observations.policy.joint_vel_history.params["asset_cfg"].joint_names = self.joint_names

        # self.observations.policy.base_lin_vel.scale = 0.  # 2.0
        ...
        self.observations.policy.base_ang_vel_history.scale = 0.25  # 0.25
        self.observations.policy.joint_pos_history.scale = 1.0  # 1.0
        self.observations.policy.joint_vel_history.scale = 0.05  # 0.05
        self.observations.policy.projected_gravity_history.scale = 0.5  # 0.5
        self.observations.policy.actions_history.scale = 1.0
        ...
        self.observations.critic.contact_state.scale = 1.0
        self.observations.critic.contact_state.params["sensor_cfg"].body_names = [self.foot_link_name]

        self.observations.policy.height_scan = None
        self.observations.critic.height_scan = None

        self.observations.privillege.base_lin_vel.scale = 2.0
        self.observations.privillege.contact_state.scale = 1.0
        self.observations.privillege.contact_state.params["sensor_cfg"].body_names = [self.foot_link_name]
        ...
        self.rewards.joint_error.weight = -0. #-1.0
        self.rewards.joint_error.params["asset_cfg"].joint_names = [f"^(?!{self.wheel_joint_name}).*"]
        self.rewards.joint_track_error.weight = 0#-1.e-3
        self.rewards.joint_track_error.params["asset_cfg"].joint_names = [f"^(?!{self.wheel_joint_name}).*"]
        self.rewards.wheel_track_error.weight = 0#-1.e-4
        self.rewards.wheel_track_error.params["asset_cfg"].joint_names = [self.wheel_joint_name]
        ...
        # Others
        self.rewards.tracking_contacts_shaped.weight = 0.
        self.rewards.tracking_contacts_shaped.params["sensor_cfg"].body_names = [self.foot_link_name]
        self.rewards.tracking_contacts_shaped.params["asset_cfg"].body_names = [self.foot_link_name]
        ...
        self.rewards.stand_still_without_cmd.weight = -0.4
        self.rewards.stand_still_without_cmd.func = mdp.wheel_stand_still_without_cmd
        self.rewards.stand_still_without_cmd.params["wheel_asset_cfg"] = SceneEntityCfg("robot", joint_names=[self.wheel_joint_name])
        self.rewards.stand_still_without_cmd.params["asset_cfg"].joint_names = [f"^(?!{self.wheel_joint_name}).*"]
        self.rewards.joint_position_penalty.weight = -0.5  # 2
        ...
        if self.__class__.__name__ == "UnitreeGo2WRoughEnvMlpWithEstimatorCfg":
