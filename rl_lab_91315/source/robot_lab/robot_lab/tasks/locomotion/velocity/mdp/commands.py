...
class UniformVelocityCommandBIT(UniformVelocityCommand):
    def __init__(self, cfg: UniformVelocityCommandBITCfg, env: ManagerBasedEnv):
        # initialize the base class
        super().__init__(cfg, env)

    def _update_command(self):
        super()._update_command()
        xy_bool = ~(torch.norm(self.vel_command_b[:, :2], dim=1) < 0.1)
        z_bool = ~(torch.abs(self.vel_command_b[:, 2]) < 0.1)
        self.vel_command_b[:, :2] *= xy_bool.unsqueeze(-1)
        self.vel_command_b[:, 2] *= z_bool

@configclass
class UniformVelocityCommandBITCfg(UniformVelocityCommandCfg):
    class_type: type = UniformVelocityCommandBIT


class Uniform6DVelocityCommandBIT(UniformVelocityCommand):
    def __init__(self, cfg: Uniform6DVelocityCommandBITCfg, env: ManagerBasedEnv):
        # initialize the base class
        super().__init__(cfg, env)
        self.default_height = cfg.ranges.default_height
        self.vel_command_b = torch.zeros(self.num_envs, 6, device=self.device)
        self.metrics["error_pose"] = torch.zeros(self.num_envs, device=self.device)
    ...
    def _update_metrics(self):
        ...
        pose = torch.cat((pitch.unsqueeze(-1), roll.unsqueeze(-1)), dim=-1)
        # logs data
        ...
        self.metrics["error_pose"] += (
            torch.norm(self.vel_command_b[:, 3:5] - pose, dim=-1) / max_command_step
        )
        self.metrics["error_height"] += (
            torch.abs(self.vel_command_b[:, 5] - self.robot.data.root_pos_w[:, 2]) / max_command_step
        )
    ...
    def _resample_command(self, env_ids: Sequence[int]):
        ...
        # -- ang pose roll
        self.vel_command_b[env_ids, 3] = r.uniform_(*self.cfg.ranges.ang_roll)
        # -- ang pose pitch
        self.vel_command_b[env_ids, 4] = r.uniform_(*self.cfg.ranges.ang_pitch)
        # -- height
        self.vel_command_b[env_ids, 5] = r.uniform_(*self.cfg.ranges.height_z)
        # heading target
    ...
    def _update_command(self):
        ...
        self.vel_command_b[standing_env_ids, 5] = self.default_height
        xy_bool = ~(torch.norm(self.vel_command_b[:, :2], dim=1) < 0.1)
        z_bool = ~(torch.abs(self.vel_command_b[:, 2]) < 0.1)
        self.vel_command_b[:, :2] *= xy_bool.unsqueeze(-1)
        self.vel_command_b[:, 2] *= z_bool
...
@configclass
class Uniform6DVelocityCommandBITCfg(UniformVelocityCommandCfg):
    class_type: type = Uniform6DVelocityCommandBIT
    ...
    class Ranges:
        ...
        ang_roll: tuple[float, float] = MISSING
        """Range for the angular-x pose command (in rad)."""

        ang_pitch: tuple[float, float] = MISSING
        """Range for the angular-y pose command (in rad)."""

        height_z: tuple[float, float] = MISSING
...
class UniformVelocityCommandTurn(UniformVelocityCommand):
    def __init__(self, cfg: UniformVelocityCommandTurnCfg, env: ManagerBasedEnv):
        # initialize the base class
        super().__init__(cfg, env)
        self.low_level = torch.zeros_like(self.is_standing_env)

    def _resample_command(self, env_ids: Sequence[int]):
        super()._resample_command(env_ids)
        r = torch.empty(len(env_ids), device=self.device)
        self.low_level[env_ids] = (~self.is_standing_env[env_ids]) & (r.uniform_(0.0, 1.0) <= self.cfg.low_level_envs)

    def _update_command(self):
        super()._update_command()
        low_level_env_ids = self.low_level.nonzero(as_tuple=False).flatten()
        # r = torch.empty(len(low_level_env_ids), device=self.device)
        self.vel_command_b[low_level_env_ids, 2] /= self.cfg.ranges.ang_vel_z[1]
        xy_bool = ~(torch.norm(self.vel_command_b[:, :2], dim=1) < 0.1)
        z_bool = ~(torch.abs(self.vel_command_b[:, 2]) < 0.1)
        self.vel_command_b[:, :2] *= xy_bool.unsqueeze(-1)
        self.vel_command_b[:, 2] *= z_bool
...
@configclass
class UniformVelocityCommandTurnCfg(UniformVelocityCommandCfg):
    class_type: type = UniformVelocityCommandTurn
    low_level_envs: float = 0.0
