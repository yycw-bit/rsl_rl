...
def randomize_motor_strength(
    ...
    env_ids: torch.Tensor | None,
    asset_cfg: SceneEntityCfg,
    leg_motor_strength_distribution_params: tuple[float, float] | None = None,
    wheel_motor_strength_distribution_params: tuple[float, float] | None = None,
    operation: Literal["add", "scale", "abs"] = "abs",
    distribution: Literal["uniform", "log_uniform", "gaussian"] = "uniform",
):
    """Randomize the motor strengths in an articulation by adding, scaling, or setting random values.

    This function allows randomizing the motor strengths
    ...
    action = env.action_manager.get_term("joint_wheel_leg")
    # action._motor_strength[:, ] =
    action._motor_strength= _randomize_prop_by_op(
        action._motor_strength.clone(),
        leg_motor_strength_distribution_params,
        env_ids,
        action._leg_mapping,
        operation,
        distribution,
    ...
    action._motor_strength = _randomize_prop_by_op(
        action._motor_strength.clone(),
        wheel_motor_strength_distribution_params,
        env_ids,
        action._wheel_mapping,
        operation,
        distribution,
    )
...
def reset_root_state_switch(
    ...
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    rand_samples[:, 4] = rand_samples[:, 4] * env.command_manager.get_command("base_velocity")[env_ids, 3]
...
def reset_joints_by_offset_switch(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    ...
    # standing_up_env_ids = cmd.command[env_ids, 3].nonzero(as_tuple=False).flatten()
    # normal_env_ids = (cmd.command[env_ids, 3] == 0).nonzero(as_tuple=False).flatten()

    # get default joint state
    joint_pos = cmd.default_joint_pos[env_ids].clone()
