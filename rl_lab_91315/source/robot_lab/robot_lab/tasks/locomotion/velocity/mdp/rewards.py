...
from isaaclab.utils.math import combine_frame_transforms, compute_pose_error, euler_xyz_from_quat
...
def wheel_stand_still_without_cmd(
    ...
    command = (torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1) < 0.1) * (torch.abs(env.command_manager.get_command(command_name)[:, 2]) < 0.1)
    return (
        (torch.sum(torch.abs(diff_angle), dim=1) + 0.5*torch.sum(torch.abs(diff_velocity), dim=1))* command  # * torch.clamp(-asset.data.projected_gravity_b[:, 2], 0, 1)
    )

def wheel_still_without_cmd(
    env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"), wheel_asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
    ) -> torch.Tensor:
    """Penalize joint positions that deviate from the default one when no command."""
    # extract the used quantities (to enable type-hinting)
    ...
    command = (torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1) < 0.1) * (torch.abs(env.command_manager.get_command(command_name)[:, 2]) < 0.1)
    return (
        torch.sum(torch.abs(diff_velocity), dim=1)* command  # * torch.clamp(-asset.data.projected_gravity_b[:, 2], 0, 1)
    )
...
def wheel_stand_still_without_cmd_stand(
    ...
    diff_angle = asset.data.joint_pos[:, asset_cfg.joint_ids] - asset.data.default_joint_pos[:, asset_cfg.joint_ids]
    diff_velocity = asset.data.joint_vel[:, wheel_asset_cfg.joint_ids]
    command = torch.norm(env.command_manager.get_command(command_name)[:, :2], dim=1) < 0.1 * (torch.abs(env.command_manager.get_command(command_name)[:, 2]) < 0.1).squeeze()
    return (
...
def track_pose_exp(env: ManagerBasedRLEnv, std: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    ...
    pose_error = torch.sum(
        torch.square(env.command_manager.get_command(command_name)[:, 3:5] - pose),
        dim=1,
    )
    return torch.exp(-pose_error / std**2)


def track_height_exp(env: ManagerBasedRLEnv, std: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    height_error = torch.square(env.command_manager.get_command(command_name)[:, 5] - asset.data.root_pos_w[:, 2])
