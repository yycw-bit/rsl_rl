...
# Pre-defined configs
##
from robot_lab.tasks.locomotion.position.config.terrains.rough import ROUGH_TERRAINS_CFG
...
class CommandsCfg:
    ...
    base_position = mdp.RobotPoseCommandCfg(
        asset_name="robot",
        resampling_time_range=(7.0, 7.0),
        debug_vis=False,
        body_name="",
        ranges=mdp.RobotPoseCommandCfg.Ranges(
            pos_x=(-1.0, 1.0),
            pos_y=(-1.0, 1.0),
            pos_z=(0.0, 0.0),
        ),
        stair_ranges = mdp.RobotPoseCommandCfg.Ranges(
            pos_x=(-0.5, 0.5),
            pos_y=(-1.0, 1.0),
            pos_z=(0.0, 0.0),
        )
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        position_commands = ObsTerm(
            func=mdp.direction_commands,
            params={"command_name": "base_position"},
            clip=(-100.0, 100.0),
            scale=1.0,
        )
        mode_boolean = ObsTerm(
            func=mdp.mode_commands,
            params={"command_name": "base_position"},
            clip=(-100.0, 100.0),
            scale=1.0,
        )
        remain_time = ObsTerm(
            func=mdp.remain_time,
            clip=(-100.0, 100.0),
            scale=1.0,
        )
        joint_pos = ObsTerm(
...
class RewardsCfg:
    ...
    track_position_xy = RewTerm(
        func=mdp.track_position_xy,
        weight=0.0,
        params={"command_name": "base_position", "time_remain_threshold": 2.0},
    )

    position_bias_xy =  RewTerm(
        func=mdp.position_bias_xy,
        weight=0.0,
        params={"command_name": "base_position", "reward_threshold": 0.5, "time_remain_threshold": 2.0},
    )

    position_bias_xyz = RewTerm(
        func=mdp.position_bias_xyz,
        weight=0.0,
        params={"command_name": "base_position", "reward_threshold": 0.5, "time_remain_threshold": 2.0},
    )

    do_not_wait = RewTerm(
        func=mdp.do_not_wait,
        weight=0.0,
        params={"command_name": "base_position"},
    )
    ...
    stand_still_without_cmd = RewTerm(
        func=mdp.wheel_stand_still_at_target,
        weight=0.0,
        params={
...
class CurriculumCfg:
    ...
    terrain_levels = CurrTerm(func=mdp.position_terrain_levels_vel,
                              params={"command_name": "base_position", "down_distance": 0.5, "up_distance": 0.3})
