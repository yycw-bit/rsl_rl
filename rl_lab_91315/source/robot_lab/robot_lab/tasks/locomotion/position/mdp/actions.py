...
class JointWheelAndLegAction(ActionTerm):
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        self._raw_actions[:] = actions
        # apply the affine transformations
        self._processed_actions[:, :-4] = self._raw_actions[:,self._leg_mapping] * self._leg_scale + self._offset
        self._processed_actions[:, -4:] = self._raw_actions[:, self._wheel_mapping] * self._wheel_scale
        # clip actions
        if self.cfg.clip is not None:
            self._processed_actions = torch.clamp(
                self._processed_actions, min=self._clip[:, :, 0], max=self._clip[:, :, 1]
